/*=========================================================================
  ...
  Module    : $Id: irtkImageFreeFormRegistrationWithPadding.cc 660 2012-08-20 17:22:48Z dr $
...
#include <irtkMultiThreadedImageFreeFormRegistrationWithPadding.h>

irtkImageFreeFormRegistrationWithPadding::irtkImageFreeFormRegistrationWithPadding()
{
  // Print debugging information
  this->Debug("irtkImageFreeFormRegistrationWithPadding::irtkImageFreeFormRegistrationWithPadding");
...
void irtkImageFreeFormRegistrationWithPadding::GuessParameter()
{
  int i;
  double xsize, ysize, zsize, spacing;

  if ((_target == NULL) || (_source == NULL)) {
    cerr << "irtkImageFreeFormRegistrationWithPadding::GuessParameter: Target and source image not found" << endl;
    exit(1);
  }
...
void irtkImageFreeFormRegistrationWithPadding::GuessParameterDistortion(double resolution, double CPS, int levels, double step, double penalty)
{
  ...
  {
    _TargetResolution[0][0] = resolution;
    _TargetResolution[0][1] = resolution;
  }
  ...
  {
    _SourceResolution[0][0] = resolution;
    _SourceResolution[0][1] = resolution;    
  }
...
{
  // Print debugging information
  this->Debug("irtkImageFreeFormRegistrationWithPadding::Initialize");
...
void irtkImageFreeFormRegistrationWithPadding::Initialize(int level)
{
  int i, j, k, n;
  double x, y, z;
  float *ptr;
  ...
  // Print debugging information
  this->Debug("irtkImageFreeFormRegistrationWithPadding::Initialize(int)");

  // Initialize base class
  this->irtkImageRegistrationWithPadding::Initialize(level);
...
void irtkImageFreeFormRegistrationWithPadding::Finalize()
{
  // Print debugging information
  this->Debug("irtkImageFreeFormRegistrationWithPadding::Finalize");
...
void irtkImageFreeFormRegistrationWithPadding::Finalize(int level)
{
  // Print debugging information
  this->Debug("irtkImageFreeFormRegistrationWithPadding::Finalize(int)");

  // Finalize base class
  this->irtkImageRegistrationWithPadding::Finalize(level);
...
void irtkImageFreeFormRegistrationWithPadding::UpdateLUT()
{
  int i, j, k;
  double x, y, z;
  float *ptr2mffd;
  ...
  // Print debugging information
  this->Debug("irtkImageFreeFormRegistrationWithPadding::UpdateLUT");
...
double irtkImageFreeFormRegistrationWithPadding::SmoothnessPenalty()
{
  int i, j, k;
...
double irtkImageFreeFormRegistrationWithPadding::SmoothnessPenalty(int index)
{
  int i, j, k;
  double x, y, z;
  double penalty=0;
...
double irtkImageFreeFormRegistrationWithPadding::VolumePreservationPenalty(int index)
{
...
double irtkImageFreeFormRegistrationWithPadding::TopologyPreservationPenalty()
{
  int i, j, k;
...
double irtkImageFreeFormRegistrationWithPadding::TopologyPreservationPenalty(int index)
{
  int i, j, k, l, m, n;
  double x, y, z, jac, penalty;
...
double irtkImageFreeFormRegistrationWithPadding::Evaluate()
{
#ifndef HAS_TBB
  // Image coordinates
  int i, j, k, t;
  ...
  // Print debugging information
  this->Debug("irtkImageFreeFormRegistrationWithPadding::Evaluate");

  // Initialize metric
  _metric->Reset();

#ifdef HAS_TBB
  irtkMultiThreadedImageFreeFormRegistrationWithPaddingEvaluate evaluate(this);
    ...
    for (k = 0; k < _target->GetZ(); k++) {
      for (j = 0; j < _target->GetY(); j++) {
        for (i = 0; i < _target->GetX(); i++) {
	      ...
	      double value = _interpolator->EvaluateInside(x, y, z, t);
	      if (value>=0)
	      {
		*ptr2tmp =  round(value);
...
double irtkImageFreeFormRegistrationWithPadding::EvaluateDerivative(int index, double step)
{
  float *ptr;
  irtkPoint p1, p2;
  ...
  // Print debugging information
  this->Debug("irtkImageFreeFormRegistrationWithPadding::EvaluateDerivative(int, double)");

#ifdef HAS_TBB
  // Create similarity metric if necessary
  if (sim_queue.try_pop(tmpMetricA) == false) {
    tmpMetricA = irtkSimilarityMetric::New(_metric);
  }
  // Create similarity metric if necessary
  if (sim_queue.try_pop(tmpMetricB) == false) {
...
double irtkImageFreeFormRegistrationWithPadding::EvaluateGradient(float step, float *dx)
{
  int i;
  double norm;
...
#ifdef HAS_TBB
  parallel_for(blocked_range<int>(0, _affd->NumberOfDOFs(), 1), irtkMultiThreadedImageFreeFormRegistrationWithPaddingEvaluateGradient(this, dx, step));
#else
...
bool irtkImageFreeFormRegistrationWithPadding::Read(char *buffer1, char *buffer2, int &level)
{
  int ok = false;
...
void irtkImageFreeFormRegistrationWithPadding::Write(ostream &to)
{
