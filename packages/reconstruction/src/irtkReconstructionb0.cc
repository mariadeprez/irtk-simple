...
irtkRealImage irtkReconstructionb0::AlignT2TemplateDist(irtkRealImage T2, bool swap)
{
  cout<<"AlignT2TemplateDist"<<endl;
  cout.flush();
  //resample T2 on the grid of reconstructed and adjust ortientation
  irtkRealImage t2= _reconstructed;
  t2=0;
  irtkRigidTransformation id;
  irtkImageTransformation *itr = new irtkImageTransformation;
  irtkImageFunction *interp = new irtkLinearInterpolateImageFunction;
  itr->SetInput(&T2, &id);
  itr->SetOutput(&t2);
  //target contains zeros, need padding -1
  itr->PutTargetPaddingValue(-1);
  //need to fill voxels in target where there is no info from source with zeroes
  itr->PutSourcePaddingValue(0);
  itr->PutInterpolator(interp);
  itr->Run();
  t2.Write("t2.nii.gz");
  t2 = AdjustOrientation(t2,swap);
  t2.Write("t2adj.nii.gz");
  
  //adjust ortientation reconstructed
  irtkRealImage rec;
  rec = AdjustOrientation(_reconstructed,swap);
  irtkAffineTransformation orient = AdjustOrientationTransformation(_reconstructed,swap);
  orient.irtkTransformation::Write("orient.dof");
  rec.Write("rec.nii.gz");
  ...
  distortion.PutStatus(SX,  _Passive);
  distortion.PutStatus(SY,  _Passive);
  distortion.PutStatus(SZ,  _Passive);
  distortion.PutStatus(SXY, _Passive);
  distortion.PutStatus(SXZ, _Passive);
  ...
  distortion.irtkTransformation::Write("translation.dof");
  ...
  irtkImageTransformation *imagetransformation = new irtkImageTransformation;
  irtkImageFunction *interpolator = new irtkLinearInterpolateImageFunction;
  imagetransformation->SetInput(&T2, &distortion);
  imagetransformation->SetOutput(&t2template);
  //target contains zeros, need padding -1
  imagetransformation->PutTargetPaddingValue(-1);
  //need to fill voxels in target where there is no info from source with zeroes
  ...
  //mask.Write("T2mask.nii.gz");
  imagetransformation->SetInput(&mask, &distortion);
  imagetransformation->SetOutput(&alignedmask);
  imagetransformation->Run();
  //alignedmask.Write("alignedT2mask.nii.gz");
  pm = alignedmask.GetPointerToVoxels();
...
void  irtkReconstructionb0::FieldMapDistortion(irtkRealImage &stack,irtkRealImage &simul, irtkMultiLevelFreeFormTransformation &distortion, bool swap, double step, int iter, int levels)
{
  ...
  if (_debug)
  {
    sprintf(buffer,"sm%i.nii.gz",iter);
    sm.Write(buffer);
    sprintf(buffer,"st%i.nii.gz",iter);
    st.Write(buffer);
  }
  ...
  */
  ...
  //distortion.irtkTransformation::Write("fmd.dof");
    cout<<"Iteration "<<iter<<": Smoothness penalty: "<<registration.SmoothnessPenalty()<<endl;
    //seg fault: cout<<"Similarity: "<<registration.Evaluate()<<endl;
  ...
  sprintf(buffer,"fmdist%i.dof",iter);
  distortion.irtkTransformation::Write(buffer);
}
