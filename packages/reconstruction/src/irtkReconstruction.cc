...
public:
    ...
    void operator()( const blocked_range<size_t>& r ) {
	    ...
	    //char buffer[256];
	    //sprintf(buffer,"tr%i.nii.gz",i);
	    //image.Write(buffer);
	    
	    //binarize s to obtain mask
            irtkRealPixel *ps = s.GetPointerToVoxels();
            for (int p = 0; p < s.GetNumberOfVoxels(); p++) {
                if (*ps != background) {
                    *ps = 1;
                }
                ps++;
            }
	    //sprintf(buffer,"s%i.nii.gz",i);
	    //s.Write(buffer);
	    
            //transform mask
	    irtkRealImage mask = image;
	    mask=0;
	    imagetransformation.SetInput(&s, &t);
            imagetransformation.SetOutput(&mask);
            imagetransformation.PutTargetPaddingValue(targetPadding);
            imagetransformation.PutSourcePaddingValue(sourcePadding);
            imagetransformation.PutInterpolator(interpolator);
            imagetransformation.Run();
	    //sprintf(buffer,"m%i.nii.gz",i);
	    //mask.Write(buffer);
...
/*
    ...
    if (resolution <= 0) {
        ...
        d = resolution;
*/
    cout << "Constructing volume with anisotropic voxel size " << attr._dx << " " <<attr._dy<<" "<<attr._dz << endl;
/*
    //resample "enlarged" to resolution "d"
    irtkNearestNeighborInterpolateImageFunction interpolator;
    irtkResampling<irtkRealPixel> resampling(d, d, d);
...
void irtkReconstruction::SetTemplateNoProcessing(irtkRealImage templateImage)
{
    _reconstructed = templateImage;
    _reconstructed.Write("Template.nii.gz");
    _template_created = true;
...
void irtkReconstruction::CreateInternalMask()
{
  //cout<<"Creating internal mask."<<endl;
...
void irtkReconstruction::SetMaskOrient(irtkRealImage * mask, irtkRigidTransformation transformation)
{
    if (!_template_created) {
        cerr
            << "Please create the template before setting the mask, so that the mask can be resampled to the correct dimensions."
...
void irtkReconstruction::SetTemplateImage(irtkRealImage t, irtkRigidTransformation tr)
{
  irtkRealImage t2template = _reconstructed;
  t2template=0;
  //irtkRigidTransformation tr;
  ...
  irtkImageTransformation *imagetransformation = new irtkImageTransformation;
  irtkImageFunction *interpolator = new irtkLinearInterpolateImageFunction;
  imagetransformation->SetInput(&t, &tr);
  imagetransformation->SetOutput(&t2template);
  //target contains zeros, need padding -1
  imagetransformation->PutTargetPaddingValue(-1);
  //need to fill voxels in target where there is no info from source with zeroes
...
void irtkReconstruction::SimulateStacks(vector<irtkRealImage>& stacks, bool simulate_excluded)
{
    if (_debug)
...
void irtkReconstruction::SetForceExcludedStacks(vector<int> &force_excluded_stacks)
{
...
void irtkReconstruction::GaussianReconstruction(double small_slices_threshold)
{
...
void irtkReconstruction::ResetBiasAndScale(vector<irtkRealImage>& bias, vector<double>& scale)
{
    if (_debug)
    ...
    for (unsigned int i = 0; i < _slices.size(); i++) {
      ...
      //Initialise voxel weights and bias values
        irtkRealPixel *pb = _bias[i].GetPointerToVoxels();
        irtkRealPixel *pi = _slices[i].GetPointerToVoxels();
        for (int j = 0; j < _bias[i].GetNumberOfVoxels(); j++) {
            if (*pi != -1) {
                *pb = 0;
            }
            else {
...
void irtkReconstruction::AddBiasAndScale(vector<irtkRealImage>& bias, vector<double>& scale)
{
    if (_debug)
...
void irtkReconstruction::InitializeEMValuesGD()
{
    if (_debug)
        cout << "InitializeEMValues" << endl;
...
public:
    ...
    void operator()( const blocked_range<size_t>& r ) const {
	    ...
	    char buffer[255];
            ...
            wresidual = 0;
	    irtkRealImage residual = wresidual;
...
/*
            sprintf(buffer,"sli%i.nii.gz",inputIndex);
            slice.Write(buffer);
            sprintf(buffer,"sim%i.nii.gz",inputIndex);
            sim.Write(buffer);
            sprintf(buffer,"residual%i.nii.gz",inputIndex);
            residual.Write(buffer);
            sprintf(buffer,"wresidual%i.nii.gz",inputIndex);
            wresidual.Write(buffer);
            sprintf(buffer,"wb%i.nii.gz",inputIndex);
            wb.Write(buffer);
*/        
            ...
            gb.Run();
/*
            sprintf(buffer,"wresidualblurred%i.nii.gz",inputIndex);
            wresidual.Write(buffer);
            sprintf(buffer,"wbblurred%i.nii.gz",inputIndex);
            wb.Write(buffer);
*/
	    //update bias field
            double sum = 0;
            ...
            for (int i = 0; i < slice.GetX(); i++)
                for (int j = 0; j < slice.GetY(); j++)
                    ...
                    /*
	    sprintf(buffer,"biasnonorm%i.nii.gz",inputIndex);
	    b.Write(buffer);
	    */
...
public:
    ...
    void operator()( const blocked_range<size_t>& r ) const {
	    ...
	    // simulated slice
            irtkRealImage simslice = reconstructor->_simulated_slices[inputIndex];
	    
	    // simulated slice ROI
	    irtkRealImage simweights = reconstructor->_simulated_weights[inputIndex];
	    ...
	    ////NEW CODE
	    //calculate average value in the slice
	    int i;
	    double average=0;
	    int n=0;
	    irtkRealPixel *ps = slice.GetPointerToVoxels();
	    ...
	    //step for GD
	    double alpha = reconstructor->_alpha_bias;
	    //if(average>0)
	      //alpha = 1/(average*average);
	    //else
	      //alpha = 0;
	    ...
	    //update _bias and calculate ROI
	    irtkRealImage bias_update(slice), bias_roi(slice);
	    bias_update=0;
	    bias_roi=0;
	    
	    ps = slice.GetPointerToVoxels();
	    irtkRealPixel *pu = bias_update.GetPointerToVoxels();
	    irtkRealPixel *pr = bias_roi.GetPointerToVoxels();
	    irtkRealPixel *psim = simslice.GetPointerToVoxels();
	    irtkRealPixel *psimw = simweights.GetPointerToVoxels();
	    irtkRealPixel *pb = b.GetPointerToVoxels();
	    irtkRealPixel *pw = w.GetPointerToVoxels();
	    
	    irtkRealImage test1(slice), test2(slice),test3(slice);
	    test1=0; test2=0; test3=0;
	    irtkRealPixel *pt1 = test1.GetPointerToVoxels();
	    irtkRealPixel *pt2 = test2.GetPointerToVoxels();
	    irtkRealPixel *pt3 = test3.GetPointerToVoxels();
	    
	    if(inputIndex==528)
	    {
	      b.Write("bias_before.nii.gz");
	    }
		...
		*pu = -alpha * *pw * *ps * (*ps * *pb  - *psim );
	    ...
	    if(inputIndex==528)
	    {
	      bias_update.Write("bias_update.nii.gz");
	      bias_roi.Write("bias_roi.nii.gz");
	      test1.Write("test1.nii.gz");
	      test2.Write("test2.nii.gz");
	      test3.Write("test3.nii.gz");
	    }
	    
	    //calculate bias field for this slice
            irtkGaussianBlurring<irtkRealPixel> gb(reconstructor->_sigma_bias);
            gb.SetInput(&bias_update);
            gb.SetOutput(&bias_update);
            gb.Run();

            gb.SetInput(&bias_roi);
            gb.SetOutput(&bias_roi);
            gb.Run();
	    
	    if(inputIndex==528)
	    {
	      bias_update.Write("bias_update_blurred.nii.gz");
	      bias_roi.Write("bias_roi_blurred.nii.gz");
	    }
	    
	    ps = slice.GetPointerToVoxels();
	    pu = bias_update.GetPointerToVoxels();
	    pr = bias_roi.GetPointerToVoxels();
	    pb = b.GetPointerToVoxels();
	    psimw = simweights.GetPointerToVoxels();
		...
		*pb=0;
    	    ...
    	    if(inputIndex==528)
	    {
	      ...
	      b.Write("bias-before-norm.nii.gz");
	    }
	    ...
	    if(inputIndex==528)
	    {
	      bias_update.Write("bias_update_final.nii.gz");
	      b.Write("bias.nii.gz");
	    }
        }
    }
             
    ParallelBiasGD( irtkReconstruction *reconstructor ) :
    reconstructor(reconstructor)
    { }
...
void irtkReconstruction::SetBiasAlpha()
{
 double average=0;
 int n=0;
 irtkRealPixel *ps;
 ...
 {
   ps = _slices[inputIndex].GetPointerToVoxels();
...
void irtkReconstruction::BiasGD()
{
    if (_debug)
        cout << "Correcting bias GD...";

    ParallelBiasGD parallelBiasGD( this );
    parallelBiasGD();
...
class ParallelLaplacianRegularization1 {
    irtkReconstruction *reconstructor;
    irtkRealImage &laplacian;
    vector<double> &factor;
    irtkRealImage &original;
        
public:
    ParallelLaplacianRegularization1( irtkReconstruction *_reconstructor,
                                     irtkRealImage &_laplacian,
                                     vector<double> &_factor,
                                     irtkRealImage &_original) : 
        reconstructor(_reconstructor),
...
class ParallelLaplacianRegularization2 {
    irtkReconstruction *reconstructor;
    irtkRealImage &laplacian;
    vector<double> &factor;
    irtkRealImage &original;
...
public:
    ParallelLaplacianRegularization2( irtkReconstruction *_reconstructor,
                                     irtkRealImage &_laplacian,
                                     vector<double> &_factor,
                                     irtkRealImage &_original) : 
        reconstructor(_reconstructor),
        ...
        for ( size_t x = r.begin(); x != r.end(); ++x ) {
		  ...
		  {                    
			  ...
			  if(reconstructor->_mask_internal(xx,yy,zz)>0)
			  {
                    ...
                    val = original(x, y, z) 
                        - reconstructor->_alpha * reconstructor->_lambda / (reconstructor->_delta * reconstructor->_delta) * val/0.068;
...
void irtkReconstruction::LaplacianRegularization(int iter, int t, irtkRealImage& original)
{
    if (_debug)
        //  cout << "AdaptiveRegularization."<< endl;
    ...
    irtkRealImage laplacian(_reconstructed);
    laplacian=0;

    ParallelLaplacianRegularization1 parallelLaplacianRegularization1( this,
                                                                     laplacian,
                                                                     factor,
                                                                     original );
    parallelLaplacianRegularization1();
    char buffer[256];
    sprintf(buffer,"laplacian%i-%i.nii.gz",iter,t);
    laplacian.Write(buffer);

    irtkRealImage original2 = _reconstructed;
    ParallelLaplacianRegularization2 parallelLaplacianRegularization2( this,
                                                                     laplacian,
                                                                     factor,
                                                                     original2 );
    parallelLaplacianRegularization2();
...
double irtkReconstruction::LaplacianSmoothness(irtkRealImage& original)
{
    CreateInternalMask();
    ...
    irtkRealPixel* pl = laplacian.GetPointerToVoxels();
    irtkRealPixel* pm = _mask_internal.GetPointerToVoxels();
    for(i=0;i<laplacian.GetNumberOfVoxels(); i++)
    {
      if(*pm==1)
      {
	sum+=*pl * *pl;
    ...
    //cout<<"Average laplacian: "<<sqrt(sum/num)<<endl;
    return sum;
}
...
class ParallelL22Regularization {
    irtkReconstruction *reconstructor;
    vector<double> &factor;
    irtkRealImage &original;
        
public:
    ParallelL22Regularization( irtkReconstruction *_reconstructor,
                                     vector<double> &_factor,
                                     irtkRealImage &_original) : 
        reconstructor(_reconstructor),
        factor(_factor),
...
void irtkReconstruction::L22Regularization(int iter, irtkRealImage& original)
{
    if (_debug)
        //  cout << "AdaptiveRegularization."<< endl;
    ...
    irtkRealImage original2 = _reconstructed;
    ParallelL22Regularization parallelL22Regularization( this,
                                                                     factor,
                                                                     original2 );
    parallelL22Regularization();
...
void irtkReconstruction::NormaliseBias(int iter)
{
    ...
    if (_debug) {
        char buffer[256];
        sprintf(buffer,"abv%i.nii.gz",iter);
        bias.Write(buffer);
        sprintf(buffer,"abvw%i.nii.gz",iter);
        _volume_weights.Write(buffer);
        sprintf(buffer,"abvm%i.nii.gz",iter);
        _mask.Write(buffer);
...
void irtkReconstruction::NormaliseBias(int iter, irtkRealImage& image)
{
    if(_debug)
        cout << "Normalise Bias ... ";
    ...
    {
      irtkRealImage stack = image.GetRegion(0,0,0,t,image.GetX(),image.GetY(),image.GetZ(),t+1);
      irtkRealPixel *pi, *pb;
      pi = stack.GetPointerToVoxels();
      pb = bias.GetPointerToVoxels();
      for (int i = 0; i<stack.GetNumberOfVoxels();i++) 
      {
        if(*pi!=-1)
            *pi /=exp(-(*pb));
        pi++;
...
public:
    ...
    ParallelNormaliseBiasGD( ParallelNormaliseBiasGD& x, split ) :
        reconstructor(x.reconstructor)
    {
        bias.Initialize( reconstructor->_reconstructed.GetImageAttributes() );
        bias = 0;    
    ...
    void join( const ParallelNormaliseBiasGD& y ) {       
        bias += y.bias;
    }
             
    ParallelNormaliseBiasGD( irtkReconstruction *reconstructor ) :
    reconstructor(reconstructor)
    {
        bias.Initialize( reconstructor->_reconstructed.GetImageAttributes() );
        bias = 0;    
...
void irtkReconstruction::NormaliseBiasGD(int iter)
{
    if(_debug)
        cout << "Normalise Bias ... ";

    ParallelNormaliseBiasGD parallelNormaliseBiasGD(this);
    parallelNormaliseBiasGD();
    irtkRealImage bias = parallelNormaliseBiasGD.bias;
...
void irtkReconstruction::HalfImage(irtkRealImage image, vector<irtkRealImage>& stacks)
{
    ...
    //We would not like single slices - that is reserved for slice-to-volume - changed that...
    if(attr._z>=2) {
...
void irtkReconstruction::PackageToVolume(vector<irtkRealImage>& stacks, vector<int> &pack_num, int iter, bool evenodd, bool half, int half_iter)
{
    ...
    for (unsigned int i = 0; i < stacks.size(); i++) {
	    ...
	    //put origin in target to zero
            ...
            m=m*mo;
            _transformations[firstSliceIndex].PutMatrix(m);
	    }//end of "if" to avoud registering empty packages
...
void irtkReconstruction::SlidingWindowRegistration(vector<irtkRealImage>& stacks, vector<int> &pack_num)
{
    irtkImageRigidRegistrationWithPadding rigidregistration;
    irtkGreyImage t,s;
    irtkRealImage target;
    ...
    int firstSlice = 0;
    cout<<"Sliding window registration: "<<endl;
    for (unsigned int i = 0; i < stacks.size(); i++) {
        cout<<"Stack "<<i<<": First slice index is "<<firstSlice<<endl;
	...
	// for all packages
        for (unsigned int j = 0; j < packages.size(); j++) {
            ...
            //resample package and save it into target
            attr=_reconstructed.GetImageAttributes();
            attr2=packages[j].GetImageAttributes();
	    //packages are not masked at present
            //irtkResamplingWithPadding<irtkRealPixel> resampling(attr._dx,attr._dx, attr2._dz,-1);
	    ...
	    //perform sliding window registration for each slice
	    for(unsigned int k = 0; k<target.GetZ(); k++)
	    {
	      //create 3 slice group around current slice in time order
	      if(k>0)
		a=k-1;
	      else
		a=0;
		...
		b=k+1;
	      ...
	      //target - group of 3 slices with current one in the middle
	      t=target.GetRegion(0,0,a,target.GetX(),target.GetY(),b+1);
              s=_reconstructed;
              
	      //identify index of this slice
              double x,y,z;
	      x=0;y=0;z=k;
	      target.ImageToWorld(x,y,z);
	      stacks[i].WorldToImage(x,y,z);
	      sliceIndex = round(z)+firstSlice;
              cout<<"Slice index: "<<sliceIndex<<endl;
	      sprintf(buffer,"sliceslidingwindow%i.nii.gz",sliceIndex);
	      t.Write(buffer);
	      ...
	      if(smax>-1)
	      {
	        ...
	        //put origin in target to zero
                ...
                ResetOrigin(t,offset);
                irtkMatrix mo = offset.GetMatrix();
                irtkMatrix m = _transformations[sliceIndex].GetMatrix();
                m=m*mo;
                _transformations[sliceIndex].PutMatrix(m);

                rigidregistration.SetInput(&t, &s);
                rigidregistration.SetOutput(&_transformations[sliceIndex]);
                rigidregistration.GuessParameterSliceToVolume();
                if(_debug)
                    rigidregistration.Write("par-sliding-window.rreg");
                rigidregistration.Run();
      
                //undo the offset
                mo.Invert();
                m = _transformations[sliceIndex].GetMatrix();
                m=m*mo;
                _transformations[sliceIndex].PutMatrix(m);
	      }//end of "if" to avoid registering empty packages
	    
              if (_debug) {
                sprintf(buffer,"transformationslidingwindow%i.dof",sliceIndex);
                _transformations[sliceIndex].irtkTransformation::Write(buffer);
              }
	    }//k - slices within package
        }//j - packages
        cout<<"End of stack "<<i<<endl<<endl;
    
        firstSlice += stacks[i].GetZ();
    }//i - stacks
}// end SlidingWindowRegistration
...
void irtkReconstruction::CropImage(irtkRealImage& image, irtkRealImage& mask, bool cropz)
{
