...
void irtkLaplacianSmoothing::Blur(irtkRealImage& image, double sigma, irtkRealImage mask, double padding)
{
  irtkGaussianBlurringWithPadding<irtkRealPixel> gb(sigma,padding);
  gb.SetInput(&image);
  gb.SetOutput(&image);
  gb.Run();
  //image.Write("blurred.nii.gz");

  gb.SetInput(&mask);
  gb.SetOutput(&mask);
  gb.Run();
  //mask.Write("blurredmask.nii.gz");
   image/=mask;
}
...
irtkRealImage irtkLaplacianSmoothing::RunGDFullRes()
{
     InitializeFactors();
