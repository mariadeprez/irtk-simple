...
int main(int argc, char **argv)
{
  ...
  for (iter=0;iter<(iterations);iter++)
  {
    ...
    //irtkAffineTransformation _translation;
    //reconstruction.ShimDistortion(original,simul,_translation,swap[0]);
    //_translation.Invert();
    //_translation.UpdateParameter();
    //sprintf(buffer,"translation%i.dof",iter);
    //_translation.irtkTransformation::Write(buffer);
    
    irtkMultiLevelFreeFormTransformation _fieldMap; //(_translation);
