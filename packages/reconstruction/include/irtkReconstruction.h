...
class irtkReconstruction : public irtkObject
{
 ...
 public:
    ...
    void SetMaskOrient(irtkRealImage * mask, irtkRigidTransformation transformation);
    void StackRegistrations(vector<irtkRealImage>& stacks, vector<irtkRigidTransformation>& stack_transformations);
    void SetTemplateImage(irtkRealImage t, irtkRigidTransformation);
    void SetTemplateNoProcessing(irtkRealImage templateImage);
   ...
   //                 int iteration );
    ...
    void CropImage( irtkRealImage& image, irtkRealImage& mask, bool cropz = true );
    ...
    void SetSlicesAndTransformations( vector<irtkRealImage>& slices,
                                      ...
                                      vector<double>& thickness );
    void SetForceExcludedStacks(vector<int> &force_excluded_stacks);
    ...
    void ResetBiasAndScale(vector<irtkRealImage>& bias, vector<double>&);
    void AddBiasAndScale(vector<irtkRealImage>& bias, vector<double>&);
    void InitializeEMValuesGD();
    ...
    void NormaliseBias(int iter, irtkRealImage& image);
    void NormaliseBiasGD(int iter);
    void SetBiasAlpha();
    ...
    ///Edge-preserving regularization with confidence map
    void L22Regularization(int iter, irtkRealImage& original);
  
    ///Edge-preserving regularization with confidence map
    void LaplacianRegularization(int iter, int t, irtkRealImage& original);
    double LaplacianSmoothness(irtkRealImage& original);
    ...
    inline irtkRigidTransformation GetTransformation(int i);
    ...
    void PackageToVolume( vector<irtkRealImage>& stacks,
  			   ...
  			   int iter,
                          ...
                          bool half=false,
                          int half_iter=1);
    void SlidingWindowRegistration(vector<irtkRealImage>& stacks, vector<int> &pack_num);
    ...
    friend class ParallelBiasGD;
    friend class ParallelScale;
    friend class ParallelNormaliseBias;
    friend class ParallelNormaliseBiasGD;
    ...
    friend class ParallelL22Regularization;
    friend class ParallelLaplacianRegularization1;
    friend class ParallelLaplacianRegularization2;
};
...
inline irtkRigidTransformation irtkReconstruction::GetTransformation(int i)
{
    return _transformations[i];
}

inline void irtkReconstruction::SetRegulSteps(int steps)
{
  _regul_steps=steps;
}

inline double irtkReconstruction::GetAverageVolumeWeight()
{
    return _average_volume_weight;
}

inline double irtkReconstruction::SetIntensityMatchingGD()
{
